...
bool IsValid(std::vector<Point2f>& track, std::vector<Point2f>& trajtrack, float& mean_x, float& mean_y, float& var_x, float& var_y, float& length)
{
	trajtrack = track;
	int size = track.size();
	float norm = 1./size;
...
bool IsCameraMotion(std::vector<Point2f>& disp)
{
	...
	float disp_norm = 1./disp_sum;//正規化
	for (int i = 0; i < disp.size(); ++i)
		disp[i] *= disp_norm;
...
void DenseSample(const Mat& grey, std::vector<Point2f>& points, const double quality, const int min_distance)
{
	...
	//コーナー検出するために，勾配行列の最小の固有値を求める
	Mat eig;	//最小固有値が格納される画像
...
// 		std::cout<<"pointsize "<<points.size()<<std::endl;
		...
		x /= min_distance;
		y /= min_distance;
		counters[y*width+x]++;//画素の番号に数を追加して目印
	}
	...
	for(int j = 0; j < width; j++, index++) {
		if(counters[index] > 0)//一度検出した点はスルー
			continue;
...
// 				std::cout<<"fx "<<float(x)<<" fy "<<float(y)<<std::endl;
	}//コーナーとして検出した点をpointsに入れて出力
}

//ピラミッド画像の作成　1/√2ずつ小さくする
//size[]の数が０の時そのまま，数が大きいほど縮小
//scales[] 何分の1倍か  ex)scales[1] = √2
...
void DrawTrack(const std::vector<Point2f>& point, const int index, const float scale, Mat& image)
{
	...
	circle(image, point0, 0.5, Scalar(0,0,255), -1, 8, 0);
}
...
void MatchFromFlow(const Mat& prev_grey, const Mat& flow, std::vector<Point2f>& prev_pts, std::vector<Point2f>& pts, const Mat& mask)
{
	...
	for(int i = 0; i < prev_pts.size(); i++) {
		...
		//test//
// 		std::cout<<" flow "<<flow.rows<<" ptr "<<flow.cols<<
// 		" width "<<width<<" f "<<f[2*(width-2)]<<std::endl;
	}
}
